...
&bull; 스크립트를 파이썬으로 학습시키기 위해 train.py를 실행시킨다. 이미지의 크기는 512 x 512, 한번에 처리되는 이미지는 16개, 학습은 300번 반복한다.<br>
해당 경로에 있는 훈련 데이터 및 검증 데이터 경로 등 정보를 포함한 data.yaml과 yolov5n.pt 파일을 사용해 훈련을 한다.
```python
